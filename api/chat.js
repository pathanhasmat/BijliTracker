...
const PLAN_LIMITS = {
  plan_free_tier:        { aiQueries: 5,   maxTenants: 2,      name: 'Free' },
  plan_starter_landlord: { aiQueries: 50,  maxTenants: 10,     name: 'Starter' },
  plan_pro_commercial:   { aiQueries: 500, maxTenants: 999999, name: 'Pro Commercial' },
};
...
function buildSystemPrompt(ctx) {
  ...
  const dataSection = isLegacy
    ...
    : `KEY LEGEND — Tenant: n=name s=status b=bills tb=total_billed tp=total_paid os=outstanding au=avg_units ld=last_bill_date
Bill: d=date t=tenant u=units a=amount p=paid s=status k=pump sl=slab
...
${monthly.length ? `HISTORY(${monthly.length} groups): ${JSON.stringify(monthly)}` : ''}`;
  ...
  return `You are BijliTracker AI — electricity bill analyst for Indian property owners.
  ...
  for (let i = 0; i < GROQ_KEYS.length; i++) {
    ...
    try {
      ...
      lastErr = err.name === 'AbortError'
        ? `key ${i + 1}: timed out after ${TIMEOUT}ms`
        : `key ${i + 1}: ${err.message}`;
